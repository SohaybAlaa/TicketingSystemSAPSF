import { getTicketsByStatusDB } from '../../_utils/db.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Query the DB for ticket counts by status, applying filters if provided
    const data = await getTicketsByStatusDB(
      startDate || null,  // Filter by start of date range
      endDate || null,    // Filter by end of date range
      team || null        // Filter by team
    );

    // Return the status breakdown to the client
    res.status(200).json({
      success: true,
      data
    });
    ...
    console.error('[API] Error fetching tickets by status:', error);
    res.status(500).json({
      success: false,
